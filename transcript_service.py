    ...
    def _extract_audio_to_wav(self, audio_url: str, wav_path: str) -> bool:
        ...
        try:
            ...
            cmd = [
                ...
                audio_url,
                "-c:a",
                "pcm_s16le",  # Force PCM WAV output
                "-ar",
                "16000",  # 16kHz sample rate
                "-ac",
                "1",  # Mono
                "-f",
                "wav",  # Force WAV format output
                wav_path,
            ]
        ...
        try:
            ...
            try:
                ...
                if transcript_obj:
                    ...
                    try:
                        if self.cookies_path and os.path.exists(self.cookies_path):
                            segments = transcript_obj.fetch(cookies=self.cookies_path)
                        elif self.cookie_header:
                            segments = transcript_obj.fetch(cookies=self.cookie_header)
                        else:
                            segments = transcript_obj.fetch()
                    except Exception as fetch_error:
                        logging.warning(
                            f"Transcript fetch failed for {video_id}: {fetch_error}"
                        )
